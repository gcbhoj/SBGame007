    ...
    var column = this.parentElement.cellIndex;
          ...
          if(clickedCards.length == 0){            // if it is the only card, add it to clickedCards, set clicked to true, set background
            ...
            clickedCards = [];  // empty the matrix.
                ...
                function showWinModal(message, onPlayAgain, onCancel) {
                    ...
                    playAgainButton.onclick = () => {
                      ...
                      onPlayAgain(); // Execute play again logic
                    };
                  
                    // Cancel button click handler
                    cancelButton.onclick = () => {
                      modal.style.display = 'none'; // Hide modal
                      onCancel(); // Execute cancel logic
                    };
                  }
                ...
                if (count == gamesize) {
                    ...
                    const winMessage = `Congratulations!! You Won in ${Sanscript.t(String(secs), 'iast', 'devanagari')} seconds while making ${Sanscript.t(String(numberOfWrongTries), 'iast', 'devanagari')} wrong moves`;
                    ...
                    showWinModal(
                      ...
                      () => {
                        console.log('Game canceled'); // Optionally handle cancel action
                      }
                    );
                  }
